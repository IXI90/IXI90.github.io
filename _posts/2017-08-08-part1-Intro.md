...
*Technical Remark* Probably you have noticed that I wasn't very precise while defining those objects. In particular, I didn't mention where the used probability measures come from. In my opinion one can get a good idea of the concepts of RL without those details. But just to give a brief hint: One can also think of a MDP as a state space with an action space and a transition probability kernel. This kernel assigns to each state-action pair a probability measure on $\mathcal{S} \times \mathbb{R}$. In this case $\mathcal{S}$ basically represents the possible following states and $\mathbb{R}$ the possible immediate rewards. This kernel gives rise to the transition and the reward function.          
