...
> Conclusion: Markov Decision Processes (MDPs) provide the right mathematical framework for Reinforcement Learning (RL) problems. In order to find the optimal policy one makes use of (optimal) state-value and (optimal) action-value functions.
