...
This definition also gives us a very intuitive partial ordering on the set $\Pi$, namely $\pi \geq \pi'$ iff $V_{\pi}(s) \geq V_{\pi'}(s)$ for $\forall s \in \mathcal{S}$. Now, we fully understand what it means for a policy to be "better" than another policy. And solving a MDP can be boiled down to a search for an optimal policy $\pi ^{a}$ (not necessarily unique) with $V_{\pi ^{a}} (s)=sup_{\pi \in \Pi} V_{\pi} (s)$ for $\forall s \in \mathcal{S}$.
