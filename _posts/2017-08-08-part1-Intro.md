...
with $s \in \mathcal{S}$. Thus $V_{\pi} (s)$ is the expected value (return), when we are in state $s$ and are going to follow policy $\pi$. Besides, we define the action-value function $Q_{\pi}: \mathcal{S} x \mathcal{A} \rightarrow \mathbb{R}$ of policy $\pi$ by  
