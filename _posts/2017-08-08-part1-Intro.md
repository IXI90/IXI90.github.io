...
Thus, acting greedily wrt the optimal action-value function gives us an optimal policy (solution to the MDP). This justifies the definition of action-value functions in general, because we now have a reasonable solution strategy for a MDP.
