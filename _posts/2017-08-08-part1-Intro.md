...
Another import term in the language of RL is "greedy"; an agent acts greedy if it takes locally optimal choices with the hope of finding a global optimum. To be more precise: The deterministic policy $\pi'$ is greedy with respect to $Q_{\pi}$ with $\pi \in \Pi$ if $\pi'(s) \in argmax_{a \in \mathcal{A}} Q_{\pi} (s, a) for all $s \in \mathcab{S}$.

Let us now assume that there exists a policy $\pi'$ that acts greedily with respect to the optimal action-value function $Q^{\ast} (e.g. possible if the action space is finite):
