...
Now, we basically have everything we need! In the following sample code I only use three simple feature maps, namely $\phi _1 (s,a) := \mathbb{1} _{v_s <0 and a=0}$, $\phi _2 (s,a) := \mathbb{1} _{v_s >0 and a=2}$ and $\phi _3 (s,a) := \mathbb{1} _{p_s >0 and a=2}$ 