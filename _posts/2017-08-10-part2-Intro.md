...
Even if the MDP is known, it is often rather complicated to calculate value functions (main reason: size of MDP). As a consequence, one searches for iterative algorithms that converge to the desired value functions. Often this is called "evaluation". 
